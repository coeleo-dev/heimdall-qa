...
# Heimdall QA — rodadas (Antigravity & Cursor)
...
```bash
./bin/heimdall-qa <comando>
# ou
.venv/bin/heimdall-qa <comando>
```

Comandos fundamentais:
* `heimdall-qa validate <round.yaml>` — valida contratos e cobertura da rodada.
* `heimdall-qa campaign validate <campaign.yaml>` — valida a cadeia de producers/placeholders de toda a campanha.
* `heimdall-qa campaign status <campaign.yaml>` — verifica o status de execução de cada rodada.
* `heimdall-qa run <round.yaml> --mode headless` — executa a rodada em modo autônomo headless (sem browser).
* `heimdall-qa last-run` — retorna o caminho do diretório `runs/latest`.
* `heimdall-qa fixture <kind>` — gera massa de dados válida (CPF, CNPJ, etc.).
...
7. `./bin/heimdall-qa validate` até 0 errors.
8. Se for revisão com operador humano: avisar para abrir a coleção via `heimdall-qa serve` (`http://127.0.0.1:7878`). **O agente nunca chama a porta 7878.**
9. Se for execução autônoma rápida solicitada: rodar `./bin/heimdall-qa run <round.yaml> --mode headless`.
...
3. `./bin/heimdall-qa campaign validate campaigns/trilho-a-http.yaml` (e `campaigns/trilho-a-live.yaml` se o pedido incluir Go-Live) até 0 errors.
4. Operador opera a árvore no browser (`heimdall-qa serve`), ou agente roda headless se expressamente pedido.
5. No fim: `./bin/heimdall-qa campaign status` + cada `runs/*-<id>/` + comentários em `verdict.json` → `analysis-campanha.md`.
...
3. `./bin/heimdall-qa validate <round.yaml>` até 0 errors (`from: ui` sem `ui.path`/`ui.read` é erro).
...
1. `./bin/heimdall-qa last-run` (ou `runs/latest`).
...
1. `./bin/heimdall-qa campaign status campaigns/trilho-a-http.yaml`.
...
Usar [`rounds/values-10m-7i.yaml`](rounds/values-10m-7i.yaml): **um** `loop` de metering, **um** `loop` de ingest (poll de status depois de cada um) e **um** `probe` — não 17 arquivos H iguais. Rate card da sessão é input do oráculo (P-GAP-4). Schema em A.18. `./bin/heimdall-qa validate rounds/values-10m-7i.yaml` e o operador abre `heimdall-qa serve rounds/values-10m-7i.yaml`. Não misturar esta rodada na campanha Trilho A HTTP.
