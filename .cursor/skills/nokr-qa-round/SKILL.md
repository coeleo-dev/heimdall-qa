...
7. `nokr-qa validate` até 0 errors.
8. Só então avisar o operador para abrir a coleção: `nokr-qa serve` (`http://127.0.0.1:7878`). Não chamar a porta 7878.
...
3. `nokr-qa campaign validate campaigns/trilho-a-http.yaml` (e `campaigns/trilho-a-live.yaml` se o pedido incluir Go-Live) até 0 errors (rounds em falta deixam de ser `missing`; placeholder sem producer, E-isolate sandbox 200, ou POST de catálogo sem `unique_json` também falham).
4. Avisar o operador: `nokr-qa serve` (ou `nokr-qa serve campaigns/trilho-a-http.yaml`). Ele opera a árvore no browser. Não chamar 7878. Não devolver uma fila de `nokr-qa serve rounds/<id>.yaml`. Live: `nokr-qa serve campaigns/trilho-a-live.yaml` com `nk_live_` em `secrets.local.yaml`.
5. No fim: `nokr-qa campaign status` + cada `runs/*-<id>/` + comentários em `verdict.json` → `analysis-campanha.md`.
...
3. `nokr-qa validate <round.yaml>` até 0 errors (`from: ui` sem `ui.path`/`ui.read` é erro).
...
1. `nokr-qa campaign status campaigns/trilho-a-http.yaml`.
...
Usar [`rounds/values-10m-7i.yaml`](../../../rounds/values-10m-7i.yaml): **um** `loop` de metering, **um** `loop` de ingest (poll de status depois de cada um) e **um** `probe` — não 17 arquivos H iguais. Rate card da sessão é input do oráculo (P-GAP-4). Schema em A.18. `nokr-qa validate rounds/values-10m-7i.yaml` e o operador abre `nokr-qa serve rounds/values-10m-7i.yaml`. Não misturar esta rodada na campanha Trilho A HTTP.
