...
SKILL = ROOT / ".cursor" / "skills" / "nokr-qa-round" / "SKILL.md"
...
def test_agents_md_covers_a14_bullets():
    ...
    assert "7878" in text
    assert "nokr-qa fixture" in text
...
def test_skill_has_frontmatter_triggers_and_protocol():
    ...
    assert "name: nokr-qa-round" in text
    ...
    assert "nokr-qa last-run" in text
    assert "nokr-qa fixture" in text
    assert "generate:" in text
...
def test_readme_documents_trilho_a_campaign():
    ...
    assert "cubra o Trilho A HTTP" in text
    assert "campaigns/trilho-a-http.yaml" in text
    assert "nokr-qa campaign validate campaigns/trilho-a-http.yaml" in text
    assert "nokr-qa campaign status campaigns/trilho-a-http.yaml" in text
    assert "nokr-qa serve\n" in text
    assert "nokr-qa fixture" in text


def test_nokrapi_agents_points_to_sibling_harness():
    text = NOKRAPI_AGENTS.read_text(encoding="utf-8")
    assert "nokr-qa" in text
    assert "7878" in text
    assert "nokr-qa-round" in text
